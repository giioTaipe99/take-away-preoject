<script setup>
import { getProductes, updateProduct, deleteProduct, addProduct, getOrders, getUsers } from './communityManager';
...
<template>
    <v-layout class="main container">
        <v-app-bar>
            ...
            <v-btn @click="showPage('page2')">ORDERS</v-btn>
            <v-btn @click="showPage('page1')">PRODUCTS</v-btn>
        </v-app-bar>
                        ...
                        <v-col v-for="product in products" :key="product.id" cols="12" md="4">
                            <v-card>
                                ...
                                <v-card-actions>
                                    ...
                                    <v-btn @click="deleteProductSelected(product.id, product.name)" color="error">Eliminar</v-btn>
                                    <v-btn @click="showDetails(product.id) " color="yellow">Info</v-btn>
                                </v-card-actions>
                                <v-expand-transition v-if="details && product.id == idProduct">
                                    <v-card>
                                        <v-parallax style="margin-right: 15px; margin-left: 15px; margin-bottom: 10px; font-weight: bold;">{{ product.name }}</v-parallax>
                                        <v-parallax style="margin-right: 15px; margin-left: 15px; margin-bottom: 10px;">Precio: {{ product.price }}€</v-parallax>
                                        <v-parallax style="margin-right: 15px; margin-left: 15px; margin-bottom: 10px;">{{ product.description }}</v-parallax>
                                    </v-card>
                                </v-expand-transition>
                            </v-card>
                        </v-col>
                        <v-card style="width: 360px; height: 323px; display: flex; align-items: center; justify-content: center; position: relative; top: 10px; margin-left: 12px;">
                            <v-btn @click="openAddProductModal" icon style="font-size: 55px; width: 80px; height: 80px;">
        ...
        <!-- P  A  G  E    O  R  D  E  R  S -->
        <Orders v-if="currentPage === 'page2'" :orders="orders" :users="users" :products="products"></Orders>
    </v-layout>
</template>
<style ></style>